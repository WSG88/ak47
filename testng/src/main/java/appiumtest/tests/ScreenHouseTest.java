...
import appiumtest.utils.ReadExcel;
import appiumtest.utils.ScreenHouseDataObject;
...
@Listeners({ AssertionListener.class }) public class ScreenHouseTest {
  // 日志对象
  Logger log = LoggerFactory.getLogger("ScreenHouseTest");
  ...
  @BeforeClass public void InitDriver() {
    ...
    // 配置线程名字用于日志记录
    Thread.currentThread().setName("ScreenHouseTest");
    // 在@test之前计划将页面到达搜索结果页再进行各个test操作
    log.info("开始筛选前先进入搜索结果页");
    // 尝试拿到搜索所有按钮，然后点击整租按钮
    // 等待首页所有按钮（整租合租等）
    ...
    log.info("首页按钮个数" + homepagebutton.size() + "点击第一个进入搜索结果页");
    // 点击首页第一个按钮（整租）（确保第一个按钮可点击）
    //WebElement button = wait.until(ExpectedConditions.elementToBeClickable(homepagebutton.get(0)));
  ...
  @Test(priority = 1) public void ScreenHouseTest() {
    log.info("现在开始测试搜索房源");
    // 拿取excel的数据
    List<ScreenHouseDataObject> ScreenHouseData = ReadExcel.readScreenHouseData();
    ...
    WebElement Region = wait.until(new ExpectedCondition<WebElement>() {
      @Override public WebElement apply(WebDriver d) {
        return SearchResult.Location(driver, "Region");
      }
    });
    // 点击区域筛选下拉
    Region.click();
    // 去获取一下地铁（筛选下拉）
    List<WebElement> RegionList = SearchResult.LocationList(driver, "RegionList");
    List<String> RegionListToString = new ArrayList<>();
    // 遍历一下，并转为string
    for (WebElement e : RegionList) {
      RegionListToString.add(e.getText());
      log.info("实际的区域下拉列表是：" + e.getText());
    }
    // 预期的区域下拉
    ...
    // 断言（查看一下区域筛选下拉选项）
    MyAssertion.AssertEquals(driver, RegionListToString, ExpectedRegionList, "区域下拉列表显示错误");
    ...
    for (int i = 0; i < ScreenHouseData.size(); i++) {
      ...
      if (ScreenNo.equals("1") || ScreenNo.equals("2")) {
        ...
        log.error("暂时还没有这个筛选,只有地铁与区域");
      }
      // 点击地铁线
      driver.findElement(By.name(ScreenHouseData.get(i).getRailway_Region())).click();
      // railwayStations表示通过id定位到的地铁站或者商圈
      List<WebElement> railwayStations = SearchResult.LocationList(driver, "railwayStation");
      ...
      while (isfind == false) {
        ...
        railwayStations = SearchResult.LocationList(driver, "railwayStation");
        for (WebElement Element : railwayStations) {
          if (Element.getText().equals(ScreenHouseData.get(i).getRailstation_Area())) {
            wait.until(ExpectedConditions.elementToBeClickable(Element));
            Element.click();
      ...
      if (ScreenHouseData.get(i).getRailstation_Area().equals("不限")) {
        // 获取筛选后区域筛选按钮的文案
        log.info("断言日志记录：筛选的是地铁线或者行政区，预期是"
            + ScreenHouseData.get(i).getRailway_Region()
            + ",实际是："
            + RegionToString);
        MyAssertion.AssertEquals(driver, RegionToString, ScreenHouseData.get(i).getRailway_Region(),
            "区域筛选器文案未更改");
      } else {
        log.info("断言日志记录：筛选的是地铁站或者是商圈，预期是"
            + ScreenHouseData.get(i).getRailstation_Area()
            + ",实际是："
            + RegionToString);
        MyAssertion.AssertEquals(driver, RegionToString,
      ...
      if (ScreenNo.equals("1")) {
        ...
        for (String actual : actual_railway) {
          log.info("预期筛选的地铁是" + RegionToString + ",实际拿到的地铁位置信息是" + actual);
          MyAssertion.AssertTrue(driver, actual.contains(RegionToString),
        ...
        for (int j = 0; j < 3; j++) {
          ...
          log.info("houselist长度" + houselist.size() + "随机数" + random_number);
          // 点击房源
          ...
          if (ScreenHouseData.get(i).getRailstation_Area().equals("不限")) {
            ...
            log.info("预期的区域：" + RegionToString + ",实际的区域：" + AddressToString[0].trim());
            MyAssertion.AssertEquals(driver, AddressToString[0].trim(), RegionToString,
                "房源详情显示的区域与搜索的不符");
          } else {
            // 商圈断言
            log.info("预期的商圈：" + RegionToString + ",实际的商圈：" + AddressToString[1].trim());
            // MyAssertold.assertequal(driver, searchkey, AddressToString[1].trim());
            MyAssertion.AssertEquals(driver, AddressToString[1].trim(), RegionToString,
                "房源详情显示的商圈与搜索的不符");
          }
          // 从房源详情页 返回房源列表页
  ...
  @Test(priority = 2) public void NearHouseTest() {
    ...
    // 点击1千米，获取距离列表
    WebElement expectedscreen = SearchResult.LocationList(driver, "railwayStation").get(1);
    String expectedscreentoString = expectedscreen.getText();
    log.info("预期筛选器文案：" + expectedscreentoString);
    ...
    log.info("实际筛选器文案：" + actualscreentoString);
    MyAssertion.AssertEquals(driver, actualscreentoString, expectedscreentoString, "筛选器文案有误");
    ...
    if (actual_distances.size() > 0) {
      for (String actual : actual_distances) {
        ...
        log.info("实际的距离值：" + actualnumber);
        MyAssertion.AssertTrue(driver, actualnumber <= 1000,
  ...
  public void UnlimitedHouseTest() {
    ...
    String actualscreentoString = Region.getText();
    log.info("实际筛选器文案：" + actualscreentoString);
    MyAssertion.AssertEquals(driver, actualscreentoString, "区域", "筛选器文案有误");
    ...
    for (WebElement actual : init_address) {
      ...
      log.info("筛选不限后，地址显示：" + actualToString);
      MyAssertion.AssertTrue(driver, !actualToString.contains("距您"),
  ...
  @AfterMethod public void afterMothod() {
    ...
    WebElement searchText = SearchResult.Location(driver, "searchText");
    while (searchText == null) {
      ...
      searchText = SearchResult.Location(driver, "searchText");
    }
  }
